...
export async function POST(req: Request) {
  ...
  const charge = await createCoinbaseCharge({
    name: `Order ${trackingCode}`,
    description: `Payment for order ${trackingCode}`,
    amount: total / 100,
    currency: "USD",
    orderId: order.id,
    redirectUrl: `${appUrl}/checkout/success?order=${order.id}&tracking=${trackingCode}`,
    cancelUrl: `${appUrl}/checkout`,
  });

  await db
    .update(orders)
    .set({ paymentRef: charge.id })
